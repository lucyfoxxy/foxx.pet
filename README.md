...
- Paragraph spacing inside panels: `--card-flow-gap` (`clamp(.25rem, .25rem + .15vw, .45rem)`).
...
- Panels scope most typography. Within `.panel` containers:
  ...
  - Paragraphs and `.prose` blocks share consistent spacing governed by `--card-flow-gap`.
